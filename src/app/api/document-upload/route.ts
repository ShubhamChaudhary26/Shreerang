...
async function processPDFandSendMail(newDoc) {
  ...
  y -= 25;
  page1.drawText(`Rent Amount: ${newDoc.rentAmount}`, {
    x: 50,
    y,
    size: 16,
    font,
  ...
  y -= 25;
  page1.drawText(`Refundable Deposit: ${newDoc.depositAmount}`, {
    x: 50,
    y,
    size: 16,
    font,
  ...
  if (newDoc.ownerAadhar) {
    y = await embedImageWithLabelFromURL(
      pdfDoc,
      page1,
      newDoc.ownerAadhar,
      "Owner Aadhar Card",
  ...
  if (newDoc.ownerPan) {
    ...
    await embedImageWithLabelFromURL(
      pdfDoc,
      page2,
      newDoc.ownerPan,
      "Owner PAN Card",
...
Rent Amount: ${newDoc.rentAmount}
Refundable Deposit: ${newDoc.depositAmount}`,
...
export async function POST(req: Request) {
  try {
    ...
    if (!name || !phone || !rentAmount || !depositAmount) {
      return NextResponse.json(
    ...
    const ownerAadhar = await uploadToCloudinary(
      form.get("ownerAadhar") as File
    );
    const ownerPan = await uploadToCloudinary(form.get("ownerPan") as File);
    const ownerIndex2 = await uploadToCloudinary(
      form.get("ownerIndex2") as File
    );
    const renterAadhar = await uploadToCloudinary(
      form.get("renterAadhar") as File
    );
